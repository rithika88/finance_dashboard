      ...
      <div className="w-full max-w-sm bg-white rounded-2xl p-8 border border-slate-100">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={loading}
            className="w-full py-2.5 rounded-lg bg-violet-600 text-white text-sm font-semibold hover:bg-violet-700 disabled:opacity-50">
            {loading ? "Logging in..." : "Log In"}
          </button>
        </form>

        <p className="text-sm text-slate-400 text-center mt-6">
          Don't have an account? <Link to="/signup" className="text-violet-600 font-medium">Sign up</Link>
        </p>
      </div>
    </div>
  );
